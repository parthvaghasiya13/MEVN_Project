<template>
  <div>
    <!-- Initial Details with the header -->
    <div id="details">
    ...
    <!-- adding image files from parsed JSON data -->
    <div id="posts">
      <div v-for="post in posts" v-bind:key="post">
        <img v-bind:src="post.download_url" alt="post.url" width="200px" height="200px" />
        <!-- adding caption of the picture, in our case: Author Name -->
...
export default {
  ...
  created() {
    axios
      ...
      .then(response => {
        //saving the data from the link to posts
